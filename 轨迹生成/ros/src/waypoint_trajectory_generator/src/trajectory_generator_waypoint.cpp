                    ...
                    Eigen::SparseMatrix<double>& hession) 
{
    ...
    int p_order = 2 * d_order - 1; //阶数
    int p_num1d = p_order + 1; //系数个数
    hession.resize(n_seg * p_num1d, n_seg * p_num1d);

    hession.setZero(); //初始化Q矩阵
    ...
    for (int k = 0; k < n_seg; ++k) {
        ...
        for (int i = d_order; i < p_num1d; ++i) {
            for (int j = d_order; j < p_num1d; ++j) {
                ...
                // SparseMatrix.insert会返回一个值的引用，将其填入就行了
                                            ...
                                            bool reverse) 
{
    ...
    int flag = d_order;
    if (one_line) { // 应该是用来给只去第一行即位置信息的，在中间约束只有位置的情况只需要第一行
        flag = 1;
    }
                                                        ...
                                                        Eigen::SparseMatrix<double>& linearMatrix) 
{
    ...
    row += n_seg - 1;
    for (int k = 0; k < n_seg - 1; ++k) {
        InsertCoff(row, k * p_num1d, linearMatrix, Time(k), d_order, false, false);//前段末尾
        InsertCoff(row, (k + 1) * p_num1d, linearMatrix, 0, d_order, false, true);//后段开头
        row += d_order;
    }
    cout << "row:" << row << endl;
            ...
            const Eigen::VectorXd &Time)          
{
    ...
    int m = Time.size();                        //即segment的段数
    MatrixXd PolyCoeff = MatrixXd::Zero(m, 3 * p_num1d);           // 因为我们要对(x,y,z)分别求解，所以要3倍的参数。
    ...
    for (int k = 0; k < m; ++k) {
        ...
        Q.block(k * p_num1d, k * p_num1d, p_num1d, p_num1d) = Q_k; //方块矩阵
    }
    
    // 计算M
    MatrixXd M = MatrixXd::Zero(m * p_num1d, m * p_num1d); //正好8个多项式系数映射到8个物理量
    ...
    // 计算C_t
    int ct_rows = d_order * 2 * m; //d的数量 2*4*seg
    int ct_cols = d_order * 2 * m - (m - 1) * d_order; // df+dp的数量 4*(seg+1)
    ...
    for (d = 0; d < d_order; d++) {
        ...
        row = std::distance(d_vector.begin(), it); // 两个迭代器的距离
        Ct(row, col) = 1;
        col += 1;
    }
    // 固定中间节点位置
    ...
    for (int dim = 0; dim < 3; dim++) {
        ...
        VectorXd d_P = -1.0 * R_pp.inverse() * R_fp.transpose() * d_F; //  通过公式求出d_P

        VectorXd d_total(d_F.rows() + d_P.rows());

        d_total << d_F, d_P;

        VectorXd poly_coef_1d = M.inverse() * Ct * d_total; // 返回求解出多项式系数
...
)        
{
    ...
    // 设置边界，求解问题，上下限用来约束不等式矩阵，弄成一样的就是等式矩阵了
