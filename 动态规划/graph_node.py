...
class Node:
    ...
    @staticmethod
    def generate_key(px, py, vx, vy):  # 应该是为了好提取状态，让其变成8字符编码，作为状态的唯一key来引索
    ...
    def connect_to_graph(self, grid):
        for u in ACTION_SPACE:  # action控制的是加速度
    ...
    def safety_constraints(self, px2, py2, grid):
        ...
        x_way_points = np.linspace(self.px, px2, step + 1, endpoint=True)  # 生成中间路径点
    ...
    def control(self, ux, uy, grid, success):
        ...
        # check collision
        status, point = self.safety_constraints(px, py, grid)  # 判定是否是合理路线
        if status == FREE:
